...
static bool CheckService(const uint256& proTxHash, const ProTx& proTx, CValidationState& state)
{
    ...
    if (Params().NetworkIDString() != CBaseChainParams::REGTEST && Params().NetworkIDString() != CBaseChainParams::TESTNET && !proTx.addr.IsRoutable()) {
        return state.DoS(10, false, REJECT_INVALID, "bad-protx-addr");
    }
...
bool CheckProRegTx(const CTransaction& tx, const CBlockIndex* pindexPrev, CValidationState& state)
{
    ...
    if (!ptx.collateralOutpoint.hash.IsNull()) {
        Coin coin;
        if (!GetUTXOCoin(ptx.collateralOutpoint, coin) || (coin.out.nValue != 2500 * COIN && coin.out.nValue != 5000 * COIN)) {
        ...
        if (tx.vout[ptx.collateralOutpoint.n].nValue != 2500 * COIN && tx.vout[ptx.collateralOutpoint.n].nValue != 5000 * COIN) {
            return state.DoS(10, false, REJECT_INVALID, "bad-protx-collateral");
        }
