...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in pyrk.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!fMnFound) {
        ...
        resultsObj.push_back(Pair("pyrk.conf", statusObj));
...
[[ noreturn ]] void gobject_help()
{
    throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in pyrk.conf\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Dash features */
    { "pyrk",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "pyrk",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "pyrk",               "gobject",                &gobject,                true,  {} },
    { "pyrk",               "voteraw",                &voteraw,                true,  {} },
