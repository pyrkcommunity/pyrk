...
UniValue getinfo(const JSONRPCRequest& request)
{
    ...
    LOCK2(cs_main, GetMainWallet() ? &GetMainWallet()->cs_wallet : nullptr);
#else
    LOCK(cs_main);
#endif
...
#ifdef ENABLE_WALLET
    if (GetMainWallet()) {
        obj.push_back(Pair("walletversion", GetMainWallet()->GetVersion()));
        obj.push_back(Pair("balance",       ValueFromAmount(GetMainWallet()->GetBalance())));
        obj.push_back(Pair("privatesend_balance",       ValueFromAmount(GetMainWallet()->GetAnonymizedBalance())));
    }
#endif
...
#ifdef ENABLE_WALLET
    if (GetMainWallet()) {
        obj.push_back(Pair("keypoololdest", GetMainWallet()->GetOldestKeyPoolTime()));
        obj.push_back(Pair("keypoolsize",   (int)GetMainWallet()->GetKeyPoolSize()));
    }
    if (GetMainWallet() && GetMainWallet()->IsCrypted()) {
        obj.push_back(Pair("unlocked_until", GetMainWallet()->nRelockTime));
    }
...
public:
    ...
    UniValue operator()(const CKeyID &keyID) const {
        ...
        if (GetMainWallet() && GetMainWallet()->GetPubKey(keyID, vchPubKey)) {
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (GetMainWallet() && GetMainWallet()->GetCScript(scriptID, subscript)) {
...
#ifdef ENABLE_WALLET
        isminetype mine = GetMainWallet() ? IsMine(*GetMainWallet(), dest) : ISMINE_NO;
        ret.push_back(Pair("ismine", bool(mine & ISMINE_SPENDABLE)));
        ret.push_back(Pair("iswatchonly", bool(mine & ISMINE_WATCH_ONLY)));
        UniValue detail = boost::apply_visitor(DescribeAddressVisitor(pwallet), dest);
        ret.pushKVs(detail);
        if (GetMainWallet() && GetMainWallet()->mapAddressBook.count(dest)) {
            ret.push_back(Pair("account", GetMainWallet()->mapAddressBook[dest].name));
        }
        CKeyID keyID;
        if (GetMainWallet()) {
            const auto& meta = GetMainWallet()->mapKeyMetadata;
            ...
            if (!keyID.IsNull() && GetMainWallet()->mapHdPubKeys.count(keyID) && GetMainWallet()->GetHDChain(hdChainCurrent)) {
                ret.push_back(Pair("hdkeypath", GetMainWallet()->mapHdPubKeys[keyID].GetKeyPath()));
...
UniValue smsglocalkeys(const JSONRPCRequest& request)
{
    ...
    if (mode == "whitelist" || mode == "all")
    {
        ...
        for (std::vector<SecMsgAddress>::iterator it = smsgAddresses.begin(); it != smsgAddresses.end(); ++it)
        {
            ...
            CPubKey pubKey;
            if (!GetMainWallet()->GetPubKey(keyID, pubKey))
                continue;
            ...
            std::string sLabel = GetMainWallet()->mapAddressBook[keyID].name;
            std::string sInfo;
            if (all)
    ...
    {
        uint32_t nKeys = 0;
        for (const auto &entry : GetMainWallet()->mapAddressBook)
        {
            if (!IsMine(*GetMainWallet(), entry.first))
                continue;
