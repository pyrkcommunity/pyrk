...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of pyrk addresses\n"
...
UniValue getblockchaininfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"size_on_disk\": \"xxxx\"     (numeric) the estimated size of the block and undo files on disk in bytes\n"
...
static UniValue getblockstats(const JSONRPCRequest& request)
{
    ...
    if (request.params[0].isNum()) {
		...
		if (!pindex) {
			try {
				...
				const int height = std::stoi(strHash);
    ...
    for (const auto& tx : block.vtx) {
        ...
        if (do_calculate_weight) {
            weight = GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION);
            total_weight += weight;
        }
