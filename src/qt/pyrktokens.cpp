...
void PyrkTokens::populateTokenInfo()
{
    ...
    if(CURLE_OK == curl_read(GetArg("-tokenapiurl", "https://tokenapi.pyrk.org/api/") + "token/" + tickerHexID, oss))
    {
                ...
                tokenStats.contains("qty_valid_meta_since_genesis"))
        {
            ...
            if (CURLE_OK == curl_read(GetArg("-tokenapiurl", "https://tokenapi.pyrk.org/api/") + "ismetaauth/" + tickerHexID + "/" + tokenAddress.ToString(), oss))
            {
...
void PyrkTokens::populateTransactionList()
{
    ...
    std::ostringstream oss;
    if(CURLE_OK == curl_read(GetArg("-tokenapiurl", "https://tokenapi.pyrk.org/api/") + "addresstransactions/" + tokenAddress.ToString() + "?limit=" + "0" /* infinite number? */, oss))
    {
        auto json = nlohmann::json::parse(oss.str());
...
void PyrkTokens::populateTokenList()
{
    ...
    std::ostringstream oss;
    if(CURLE_OK == curl_read(GetArg("-tokenapiurl", "https://tokenapi.pyrk.org/api/") + "address/" + tokenAddress.ToString(), oss))
    {
        auto json = nlohmann::json::parse(oss.str());
...
{
    pwalletMain->NotifyTransactionChanged.connect(boost::bind(NotifyTransactionChanged, this, _1, _2, _3));
}

void PyrkTokens::unsubscribeFromCoreSignals()
{
    pwalletMain->NotifyTransactionChanged.disconnect(boost::bind(NotifyTransactionChanged, this, _1, _2, _3));
}
