...
bool selectInput(std::set<CBitcoinAddress>& setAddress, uint256& inputtxid, int& inputvout, CAmount& inputvalue, CAmount selectvalue)
{
    ...
    vpwallets[0]->AvailableCoins(vecOutputs, false, nullptr, true);
...
bool sendTokenTransaction(CMutableTransaction& rawTx)
{
    ...
    SignSignature(keystore, prevPubKey, rawTx, 0, amount, SIGHASH_ALL);

    ScriptError serror = SCRIPT_ERR_OK;
    if (!VerifyScript(txin.scriptSig, prevPubKey, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, 0, amount), &serror)) {
        QMessageBox::critical(nullptr, QMessageBox::tr("Error"),
    ...
    if (!fHaveMempool && !fHaveChain) {
        ...
        bool fMissingInputs;
        if (!AcceptToMemoryPool(mempool, state, tx, !fBypassLimits, &fMissingInputs, false, false, nMaxRawTxFee)) {
            if (state.IsInvalid()) {
                QMessageBox::critical(nullptr, QMessageBox::tr("Error"),
