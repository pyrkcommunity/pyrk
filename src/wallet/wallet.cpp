...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                bool found = false;
                COutPoint outbind(pcoin->tx->GetHash(), i);
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    found = !CPrivateSend::IsDenominatedAmount(pcoin->tx->vout[i].nValue);
                } else if (validateCollateral(outbind)) {
                    found = true;
                ...
                if (!(IsSpent(wtxid, i)) && mine != ISMINE_NO &&
                    (!IsLockedCoin((*it).first, i) || validateCollateral(outbind)) &&
    ...
    for (const auto& outpoint : setWalletUTXO) {
        ...
        for (unsigned int i = 0; i < wtx.tx->vout.size(); i++) {
            ...
            if(fAnonymizable) {
                ...
                if(fMasternodeMode && (wtx.tx->vout[i].nValue == 1000*COIN || wtx.tx->vout[i].nValue == 2500*COIN || wtx.tx->vout[i].nValue == 5000*COIN)) continue;
...
bool CWallet::SelectPrivateCoins(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& vecTxInRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax) const
{
    ...
    for (const auto& out : vCoins)
    {
        ...
        if(fMasternodeMode && (out.tx->tx->vout[out.i].nValue == 1000*COIN || out.tx->tx->vout[out.i].nValue == 2500*COIN || out.tx->tx->vout[out.i].nValue == 5000*COIN)) continue;
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 PYRK."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
