...
#if defined(NDEBUG)
# error "Pyrk Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nHeight, bool fSuperblockPartOnly)
{
    ...
    // Return superblock payment or block subsidy less superblock amount
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().BIP147Height) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            for (const auto& txin : tx->vin) {
                ...
                if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                    ...
                    return state.DoS(10, error("ConnectBlock(): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    LogPrintf("%s: new best=%s height=%d version=0x%08x algo=%d (%s) log2_work=%.8f tx=%lu date='%s' progress=%f cache=%.1fMiB(%utxo)", __func__,
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    int nLockTimeFlags = 0;
    if (nHeight >= Params().GetConsensus().CSVHeight) {
